...
tape( 'the function has an arity of 4', function test( t ) {
	t.strictEqual( gxsa.length, 4, 'has expected arity' );
	t.end();
});

tape( 'the function subtracts a constant from each element of a strided array', function test( t ) {
	var expected;
	var x;
...
tape( 'the function subtracts a constant from each element of a strided array (accessors)', function test( t ) {
	var expected;
	var x;
	...
	gxsa( x.length, 5.0, toAccessorArray( x ), 1 );
	t.deepEqual( x, expected, 'returns expected value' );
...
tape( 'the function supports specifying a stride (accessors)', function test( t ) {
	...
	gxsa( 3, 5.0, toAccessorArray( x ), 2 );
	t.deepEqual( x, expected, 'returns expected value' );
	t.end();
});
