...
var gxsa = require( './../lib/ndarray.js' );
...
tape( 'the function has an arity of 5', function test( t ) {
	t.strictEqual( gxsa.length, 5, 'has expected arity' );
	t.end();
});
...
tape( 'the function subtracts a constant from each strided array element (accessors)', function test( t ) {
	var expected;
	var x;
	...
	gxsa( x.length, 5.0, toAccessorArray( x ), 1, 0 );
	t.deepEqual( x, expected, 'returns expected value' );
...
tape( 'the function supports specifying a stride (accessors)', function test( t ) {
	...
	gxsa( 3, 5.0, toAccessorArray( x ), 2, 0 );
	t.deepEqual( x, expected, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports specifying a negative stride (accessors)', function test( t ) {
	...
	gxsa( 3, 5.0, toAccessorArray( x ), -2, x.length-1 );
	t.deepEqual( x, expected, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports an offset parameter (accessors)', function test( t ) {
	...
	gxsa( 3, 5.0, toAccessorArray( x ), 2, 1 );
	t.deepEqual( x, expected, 'returns expected value' );
	t.end();
});

tape( 'if `stride` is equal to `1`, the function efficiently subtracts a constant from each element in a strided array', function test( t ) {
	var expected;
	var alpha;
	var x;
	var i;
